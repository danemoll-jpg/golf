// Firestore-backed "room" — the one shared document two (or more) browsers use to find
// each other and, once a match starts, to sync turns. Same pattern as Durak's
// packages/client/src/network/rooms.ts (itself adapted from Par Five's src/network/rooms.js).
//
...
//     gameState: null | GameState,  // the full engine state, including every still-face-down
//       card — see the README's "Card privacy" note: this is knowingly readable by anyone
//       with the room code, same trust model as Durak's hand privacy / Par Five's public docs.
 ...
 * uses `isBot: boolean`), converted between the two only at startMatch(). `icon` is the
...
/** Host action: deals hole 1 (using whatever house rules are currently set on the room)
 * and moves the room into "playing". */
 ...
 * produced. Called by whoever computed the move locally — see useOnlineRoom. Returns the
 * new commentary array/seq so a caller stepping several moves in a row (the host driving a
 * run of bot turns) can keep its own local copy accurate between snapshot round-trips,
 * instead of re-deriving from a stale `room` and clobbering just-written lines. */
...
/** Marks a seat ready for the next hole during the 'holeOver' pause. Unlike every other move
 * in the game, ready-ing up is NOT turn-based — any number of players can legitimately click
 * it within the same pause, each from their own possibly-stale local `gameState`. A plain
 * writeGameState() (read-nothing, blind overwrite) would let two humans clicking within the
 * same round-trip silently clobber one another's ready flag and hang the room waiting on
...
export async function sendReadyForNextHole(code: string, seatId: string, provider: CommentaryProvider): Promise<void> {
  await runTransaction(db, async (tx) => {
    const ref = roomRef(code);
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error('Room not found.');
    ...
    const next = applyAction(room.gameState, seatIndex, { type: 'readyForNextHole' });
    // Readying up can, once everyone's in, cascade straight into dealing the next hole or
    // ending the match — same commentary-worthy events (holeStarted, matchOver, …) those
...
/** Resets a room back to its lobby, keeping the same seats — "New match" without leaving
 * the room. */
