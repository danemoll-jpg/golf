...
/** Shows one commentary line at a time, in order, instead of stacking several toasts at
 * once — a burst of events (several great plays in a row, a hole ending) used to pop up
 * multiple lines simultaneously with independent timers, so they'd all vanish together
 * before there was time to read more than one. `entries` is already a chronological queue
 * (the hooks only ever append, never reorder) — showing just entries[0] and dismissing it
 * after DISPLAY_MS naturally plays the backlog one line at a time instead of all at once. */
