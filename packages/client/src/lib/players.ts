...
import { BotPersonalityId, EngineConfig, PERSONALITIES } from '@golf/engine';
...
/** Capped at 4 seats total — matches Durak's table cap. Only two bot personalities exist
 * today (Ed, Carol), so a room can have at most 2 bot seats regardless of this cap
 * (nextBotPersonality below returns null once both are taken). Add more entries to
 * PERSONALITIES in @golf/engine and raise this cap further to support bigger all-bot
 * tables. */
 ...
 * and the in-game player badges. Bots always show their personality's fixed avatar; humans
 * show whatever they picked (see lib/icons.ts), falling back to the default if unset (e.g.
 * an unclaimed open seat). */
