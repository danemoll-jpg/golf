...
import { applyAction, BotDifficulty, chooseBotAction, GameEvent, GameState } from '@golf/engine';
 ...
 * difficulty (defaults to 'normal' if unspecified). A single call may only draw (leaving
 * the same bot still acting to decide swap/discard next) — the caller loops via isBotTurn
 * until it's a human's turn again. */
