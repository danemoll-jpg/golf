...
/** Runs a Golf match entirely in the browser — no server, no network. Local (vs-bots) play
...
export function useLocalGame(): UseLocalGame {
  ...
  const startMatch = useCallback(
    (humanName: string, totalPlayers: number, icon: string, jokers: boolean) => {
      ...
      const fresh = createMatch({ playerConfigs: buildPlayerConfigs(seats), rules: { jokers } });
      setState(fresh);
      (async () => {
