...
/**
 * Swappable commentary source. `TemplateCommentaryProvider` (the default) picks
 * randomized canned lines with no network calls. A future `ClaudeCommentaryProvider` can
 * implement this same interface — using the Claude API to generate the line text instead —
 * and be swapped in via config, with zero changes to game logic.
 */
